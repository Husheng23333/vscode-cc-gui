...
export function useDialogManagement({ t }: UseDialogManagementOptions): UseDialogManagementReturn {
  ...
  const closePermissionDialogIfCurrent = useCallback((channelId: string, dialogToken?: string) => {
    pendingPermissionRequestsRef.current = pendingPermissionRequestsRef.current.filter(
      (item) => item.channelId !== channelId || (dialogToken !== undefined && item.dialogToken !== dialogToken)
    );
  ...
  const closeAskUserQuestionDialogIfCurrent = useCallback((requestId: string, dialogToken?: string) => {
    pendingAskUserQuestionRequestsRef.current = pendingAskUserQuestionRequestsRef.current.filter(
      (item) => item.requestId !== requestId || (dialogToken !== undefined && item.dialogToken !== dialogToken)
    );
  ...
  const closePlanApprovalDialogIfCurrent = useCallback((requestId: string, dialogToken?: string) => {
  ...
  const handlePermissionApprove = useCallback((channelId: string) => {
    ...
    closePermissionDialogIfCurrent(channelId, currentPermissionRequestRef.current?.dialogToken);
  ...
  const handleAskUserQuestionSubmit = useCallback((requestId: string, answers: Record<string, string | string[]>) => {
    ...
    closeAskUserQuestionDialogIfCurrent(requestId, currentAskUserQuestionRequestRef.current?.dialogToken);
  ...
  const handlePlanApprovalApprove = useCallback((requestId: string, targetMode: string) => {
    ...
    closePlanApprovalDialogIfCurrent(requestId, currentPlanApprovalRequestRef.current?.dialogToken);

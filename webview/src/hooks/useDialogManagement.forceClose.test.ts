...
describe('useDialogManagement - forceClose queue draining (issue #1360)', () => {
  const mkAsk = (requestId: string): AskUserQuestionRequest => ({ requestId, toolName: 'test', questions: [] });
  const mkPermission = (channelId: string): PermissionRequest => ({ channelId, toolName: 'test', inputs: {} });
  const mkPlan = (requestId: string): PlanApprovalRequest => ({ requestId, toolName: 'test' });

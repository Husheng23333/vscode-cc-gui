...
describe('ModelConfigSelect', () => {
  ...
  describe('submenu hover delay', () => {
    ...
    it('does not steal the effort submenu while the pointer crosses the context row', () => {
      render(
        <ModelConfigSelect
          selectedModel="claude-sonnet-5"
          onModelSelect={vi.fn()}
      ...
      // Crossing the context row arms a delayed dismiss of the fly-out.
      fireEvent.mouseEnter(screen.getByTestId('model-config-option-context'));
      act(() => {
    ...
    it('dismisses the effort fly-out when the pointer moves onto the flat model list', () => {
      ...
      fireEvent.mouseEnter(document.querySelector('.model-config-models') as HTMLElement);
      act(() => {
        vi.advanceTimersByTime(SUBMENU_HOVER_DELAY_MS);
      });
      expect(screen.queryByTestId('reasoning-selector-dropdown')).toBeNull();
    ...
    it('opens the effort submenu after the pointer rests on it, or immediately on click', () => {
      render(
        <ModelConfigSelect
          selectedModel="claude-sonnet-5"
          onModelSelect={vi.fn()}
      ...
      // Resting on the context row dismisses the fly-out after the switch delay.
      fireEvent.mouseEnter(screen.getByTestId('model-config-option-context'));
      act(() => {
        vi.advanceTimersByTime(SUBMENU_HOVER_DELAY_MS);
      });

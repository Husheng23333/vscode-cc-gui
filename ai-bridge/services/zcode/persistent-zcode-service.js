...
let activeTurn = null;
/** Daemon request id owning the active turn (multi-window scoped abort). */
...
function waitForTurn(sessionId, normalizer, requestId = null) {
...
export async function sendMessagePersistent(params = {}) {
  ...
  // Capture the daemon request id synchronously (queued/async work below may
  // run after this request's ALS context unwinds) so a scoped abort can match
  // this turn — same multi-window pattern as Grok ACP / DSH.
  ...
  const turnOutcome = waitForTurn(sessionId, normalizer, requestId);
...
export async function abortCurrentTurn(targetRequestIds) {
  ...
  // Multi-window scoping: when the extension passes target request ids, only
  // abort if this turn's owning request is among them, so one window's stop
  // cannot kill another window's ZCode turn. Undefined aborts whatever turn
  // is active (legacy abort-all). Mirrors grok abortCurrentTurn scoping.
